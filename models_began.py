...
class D(nn.Module):
    def __init__(self, h, n, input_dim=(64,64,3)):
        ...
        self.fc_decode = nn.Linear(h + 128, 8 * 8 * n)
    ...
    def forward(self,x, encoded_caption):
        #   encoder        [ flatten ] 
        x = self.encoder(x).view(x.size(0), -1)
        # print(x)
        x = self.fc_encode(x)

        x = torch.cat((x, encoded_caption), 1)
...
class EncoderRNN(nn.Module):
    ...
    def forward(self, captions, lengths):
        ...
        out = torch.cat([ unpacked[ i, lengths[i] - 1 ].unsqueeze(0) for i in range( len(lengths) ) ], 0)
