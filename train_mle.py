...
import numpy as np
import os
from data_loader_coco_aesthetics import get_loader 
...
def run(save_path, args):
    ...
    # Build the models
    encoder = EncoderCNN(args.embed_size,models.inception_v3(pretrained=True), requires_grad=args.finetune)
    ...
    if args.finetune:
        params.append({'params': encoder.parameters(), 'lr': 0.5 * args.learning_rate})
    ...
    for epoch in range(args.num_epochs):
        for i, (images, captions, lengths, img_id) in enumerate(train_data_loader):
            ...
            netG.zero_grad()
            inputs = Variable(images.data, volatile= not args.finetune)
            if args.finetune:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tb_log_step', type=int , default=100,
                        help='step size for prining log info')
    parser.add_argument('--save_step', type=int , default=8351,
    ...
    parser.add_argument('--netG', type=str, default='logs/aesthetics/16052017144959/netG-1-8351.pkl')
    parser.add_argument('--encoder', type=str, default='logs/aesthetics/16052017144959/encoder-1-8351.pkl')
